...
import uk.gov.hmrc.alcoholdutyaccount.models.UserFulfilledObligations
import uk.gov.hmrc.mongo.test.DefaultPlayMongoRepositorySupport

import java.time.temporal.ChronoUnit

class UserFulfilledObligationsRepositorySpec
    extends ISpecBase
    with DefaultPlayMongoRepositorySupport[UserFulfilledObligations] {
  ...
  protected override val repository: UserFulfilledObligationsRepository = new UserFulfilledObligationsRepository(
  ...
  "get must" - {
    "get the record for the appaId when it exists" in {
      insert(userFulfilledObligations).futureValue

      val result = repository.get(appaId).futureValue
      verifyResult(result.value, userFulfilledObligations)
    }
  ...
  "set must" - {
    "replace the record for the given appaId" in {
      insert(userFulfilledObligations.copy(fulfilledObligationsData = Seq(fulfilledObligations2025))).futureValue

      val setResult   = repository.set(userFulfilledObligations).futureValue
      val addedRecord = find(Filters.equal("_id", appaId)).futureValue.headOption.value

      setResult mustEqual userFulfilledObligations
      verifyResult(addedRecord, userFulfilledObligations)
    }
  ...
  "deleteAll must" - {
    "delete all records from the collection" in {
      insert(userFulfilledObligations).futureValue
      insert(userFulfilledObligations.copy(appaId = "test")).futureValue
  ...
  def verifyResult(actual: UserFulfilledObligations, expected: UserFulfilledObligations): Assertion = {
    actual.appaId                                   mustEqual expected.appaId
    actual.fulfilledObligationsData                 mustEqual expected.fulfilledObligationsData
